...
resource "aws_cloudwatch_log_group" "obsidian_waf_log_group01" {
...
resource "aws_wafv2_web_acl_logging_configuration" "obsidian_waf_logging01" {
  count = var.enable_waf && var.waf_log_destination == "cloudwatch" ? 1 : 0

  resource_arn = aws_wafv2_web_acl.obsidian_waf01[0].arn
  log_destination_configs = [
    aws_cloudwatch_log_group.obsidian_waf_log_group01[0].arn
  ]

  # TODO: Students can add redacted_fields (authorization headers, cookies, etc.) as a stretch goal.
  # redacted_fields { ... }

  depends_on = [aws_wafv2_web_acl.obsidian_waf01]
}
...
# Explanation: S3 WAF logs are the long-term archive—Obsidian likes receipts that survive dashboards.
resource "aws_s3_bucket" "obsidian_waf_logs_bucket01" {
  count = var.waf_log_destination == "s3" ? 1 : 0

  bucket = "aws-waf-logs-${var.project_name}-${data.aws_caller_identity.obsidian_self01.account_id}"
...
resource "aws_s3_bucket_public_access_block" "obsidian_waf_logs_pab01" {
  count = var.waf_log_destination == "s3" ? 1 : 0

  bucket                  = aws_s3_bucket.obsidian_waf_logs_bucket01[0].id
...
resource "aws_wafv2_web_acl_logging_configuration" "obsidian_waf_logging_s3_01" {
  count = var.enable_waf && var.waf_log_destination == "s3" ? 1 : 0

  resource_arn = aws_wafv2_web_acl.obsidian_waf01[0].arn
  log_destination_configs = [
    aws_s3_bucket.obsidian_waf_logs_bucket01[0].arn
  ]
...
resource "aws_s3_bucket" "obsidian_firehose_waf_dest_bucket01" {
  count = var.waf_log_destination == "firehose" ? 1 : 0

  bucket = "${var.project_name}-waf-firehose-dest-${data.aws_caller_identity.obsidian_self01.account_id}"
...
# Explanation: Firehose needs a role—Obsidian doesn't let random droids write into storage.
resource "aws_iam_role" "obsidian_firehose_role01" {
  count = var.waf_log_destination == "firehose" ? 1 : 0
...
resource "aws_iam_role_policy" "obsidian_firehose_policy01" {
  count = var.waf_log_destination == "firehose" ? 1 : 0
  name  = "${var.project_name}-firehose-policy01"
  role  = aws_iam_role.obsidian_firehose_role01[0].id
    ...
    Statement = [
      {
        ...
        Resource = [
          aws_s3_bucket.obsidian_firehose_waf_dest_bucket01[0].arn,
          "${aws_s3_bucket.obsidian_firehose_waf_dest_bucket01[0].arn}/*"
        ]
      }
    ]
...
resource "aws_kinesis_firehose_delivery_stream" "obsidian_waf_firehose01" {
  count       = var.waf_log_destination == "firehose" ? 1 : 0
  name        = "aws-waf-logs-${var.project_name}-firehose01"
  destination = "extended_s3"

  extended_s3_configuration {
    role_arn   = aws_iam_role.obsidian_firehose_role01[0].arn
    bucket_arn = aws_s3_bucket.obsidian_firehose_waf_dest_bucket01[0].arn
    prefix     = "waf-logs/"
  }
}

# Explanation: Connect shield generator to conveyor belt—WAF -> Firehose stream.
resource "aws_wafv2_web_acl_logging_configuration" "obsidian_waf_logging_firehose01" {
  count = var.enable_waf && var.waf_log_destination == "firehose" ? 1 : 0

  resource_arn = aws_wafv2_web_acl.obsidian_waf01[0].arn
  log_destination_configs = [
    aws_kinesis_firehose_delivery_stream.obsidian_waf_firehose01[0].arn
  ]
